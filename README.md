...
* **Suite de Pruebas Automatizadas con Fixtures de Oro:** 130 tests unitarios de integración continua (`npm test`) que validan toda la cadena operativa, cálculo matemático, motores de señal, normalización temporal, scoring bayesiano, invarianza de $PF_R$, hash FNV-1a de caché, aislamiento estricto In-Sample/OOS sin data leakage, compuertas de multiplicidad, rampa continua de sobreextensión ($1.8 \to 2.2$ ATR), compatibilidad geométrica con veto direccional de VWAP (aplicado exclusivamente en zona ordinaria $|\text{distAtr}| < 1.8$ y eximiendo la reversión a la media en sobreextensiones extremas), calibración empírica del umbral de Scoring ($0.45 \times \text{maxPossible} \implies 3.15$ en 5m / $3.83$ en 1H) testeada sobre datos de mercado reales de la watchlist para mantener $\ge 80\% - 100\%$ de símbolos sobre el piso institucional `minHighResolved` sin sobreoperar con ruido, folds disjuntos clasificados con distinción NO_DATA vs FAIL, degradación a LIMITED ante ausencia total de operaciones en folds, gestión unificada de gaps de sesión (SESSION_GAP con exclusión de la barra de ejecución $f > \text{entryCandleIdx} + 1$), alineación activa de pendiente RSI (RSI Slope) en Standard Voting contra cuchillos cayientes, piso estricto de calentamiento de indicadores ($oldestEvalIdx \ge 30$) eliminando sesgos a ranging por ADX NaN, enrutamiento canónico unificado de forwardWindow y partición Walk-Forward de punta a punta.
