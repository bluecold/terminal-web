...
* **Suite de Pruebas Automatizadas con Fixtures de Oro:** 133 tests unitarios de integración continua (`npm test`) que validan toda la cadena operativa, cálculo matemático, motores de señal, normalización temporal, scoring bayesiano, invarianza de $PF_R$, hash FNV-1a de caché, aislamiento estricto In-Sample/OOS sin data leakage, compuertas de multiplicidad, mapeo continuo y estrictamente monotónico en Capa 4 de Scoring ($s_4 = \tanh(\text{distAtr})$) con veto direccional incondicional de VWAP en intradía (eliminando señales contra-tendencia en capitulaciones intradiarias), calibración empírica unificada del umbral de Scoring ($0.45 \times \text{maxPossible} \implies 3.15$ en 5m / $3.83$ en 1H) con paridad matemática estricta $1:1$ entre Backtest y Escáner en Vivo vía `DEFAULT_SCORING_THRESHOLD_RATIO`, derivación canónica estricta e invariable del multiplicador ATR y forward window vía `getStrategyAtrMultiplier` (eliminando puertas traseras de desacoplamiento de riesgo), cálculo canónico de Profit Factor vía `calculateProfitFactor` con paridad estricta en estados vacíos (`null` en 0 trades eliminando falsos breakeven $1.0$), eliminación de sesgos autorreferenciales en ventanas rodantes de percentiles (`calculateAndianOscillator`, `calculateBollingerVolatilityStatus`, `calculateRevolutionVolatilityBand`), unificación canónica de la detección de saltos de sesión en la vela de ejecución vía `isExecutionAcrossSessionGap` (reemplazando números mágicos como 900s en VCME y MTF), alineación canónica de `oldestEvalIdx` con el suelo de warmup de la estrategia en todos los timeframes (`getStrategySignalWarmup`: 34 Estándar, 30 Confluencia, 59 Scoring) eliminando el prefijo estéril de la ventana In-Sample y sesgos a ranging por ADX NaN, folds disjuntos clasificados con distinción NO_DATA vs FAIL, degradación a LIMITED ante ausencia total de operaciones en folds, gestión unificada de gaps de sesión (SESSION_GAP con exclusión de la barra de ejecución $f > \text{entryCandleIdx} + 1$), alineación activa de pendiente RSI (RSI Slope) en Standard Voting contra cuchillos cayientes, enrutamiento canónico unificado de forwardWindow y partición Walk-Forward de punta a punta.
