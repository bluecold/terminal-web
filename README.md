...
* **Suite de Pruebas Automatizadas con Fixtures de Oro:** 128 tests unitarios de integración continua (`npm test`) que validan toda la cadena operativa, cálculo matemático, motores de señal, normalización temporal, scoring bayesiano, invarianza de $PF_R$, hash FNV-1a de caché, aislamiento estricto In-Sample/OOS sin data leakage, compuertas de multiplicidad, rampa continua de sobreextensión, folds disjuntos clasificados con distinción NO_DATA vs FAIL, gestión unificada de gaps de sesión (SESSION_GAP con exclusión de la barra de ejecución $f > \text{entryCandleIdx} + 1$), paridad canónica 1:1 de forwardWindow/alertTracker, paridad de cooldown, calendario bursátil y partición Walk-Forward de punta a punta.
