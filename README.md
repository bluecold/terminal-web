...
* **Suite de Pruebas Automatizadas con Fixtures de Oro:** 131 tests unitarios de integración continua (`npm test`) que validan toda la cadena operativa, cálculo matemático, motores de señal, normalización temporal, scoring bayesiano, invarianza de $PF_R$, hash FNV-1a de caché, aislamiento estricto In-Sample/OOS sin data leakage, compuertas de multiplicidad, rampa continua de sobreextensión ($1.8 \to 2.2$ ATR), compatibilidad geométrica con veto direccional de VWAP (aplicado exclusivamente en zona ordinaria $|\text{distAtr}| < 1.8$ y eximiendo la reversión a la media en sobreextensiones extremas), calibración empírica del umbral de Scoring ($0.45 \times \text{maxPossible} \implies 3.15$ en 5m / $3.83$ en 1H) anclada a evidencia real, alineación canónica de `oldestEvalIdx` con el suelo de warmup de la estrategia en todos los timeframes (`getStrategySignalWarmup`: 34 Estándar, 30 Confluencia, 59 Scoring) eliminando el prefijo estéril de la ventana In-Sample y sesgos a ranging por ADX NaN, folds disjuntos clasificados con distinción NO_DATA vs FAIL, degradación a LIMITED ante ausencia total de operaciones en folds, gestión unificada de gaps de sesión (SESSION_GAP con exclusión de la barra de ejecución $f > \text{entryCandleIdx} + 1$), alineación activa de pendiente RSI (RSI Slope) en Standard Voting contra cuchillos cayientes, enrutamiento canónico unificado de forwardWindow y partición Walk-Forward de punta a punta.
