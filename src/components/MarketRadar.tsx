  ...
  const scanSymbol = async (symbol: string, forceFresh: boolean = false): Promise<RadarRowData> => {
    ...
    try {
      ...
      const candidates: StrategyCandidate[] = [
        ...
        { key: 'multifractal', label: 'Multifractal MTF', profitFactor: btMF ? btMF.profitFactor : null, expectancyR: btMF ? btMF.expectancyR : 0, expectancyPerHour: btMF ? btMF.expectancyPerHour : 0, avgExposureHours: btMF ? btMF.avgExposureHours : 0, winRate: btMF ? btMF.winRate : 0.5, resolved: btMF ? (btMF.totalSignals > 0 ? btMF.totalSignals : btMF.wins + btMF.losses) : 0, maxDrawdownR: btMF?.maxDrawdownR, sortinoRatio: btMF?.sortinoRatio, maxLossStreak: btMF?.maxLossStreak, longStats: btMF?.longStats, shortStats: btMF?.shortStats, regimeStats: btMF?.regimeStats, forwardWindow: 12 },
      ];
